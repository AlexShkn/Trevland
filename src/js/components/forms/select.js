...
export const selectModule = {
	...
	getSelectElement(selectItem, className) {
		return {
			...
			selectElement: selectItem.querySelector(
				selectModule.getSelectClass(className),
			),
		}
	},
	...
	selectBuild(originalSelect) {
		...
		originalSelect.multiple
			...
			: selectItem.classList.remove(
					selectModule.selectClasses.classSelectMultiple,
			  )
		...
		originalSelect.hasAttribute('data-checkbox') && originalSelect.multiple
			...
			: selectItem.classList.remove(
					selectModule.selectClasses.classSelectCheckBox,
			  )
		...
		originalSelect.hasAttribute('data-search')
			? selectModule.searchActions(selectItem)
			: null
	...
	selectsActions(e) {
		...
		if (
			targetElement.closest(
				selectModule.getSelectClass(selectModule.selectClasses.classSelect),
			) ||
			targetElement.closest(
						...
						`.${selectModule.selectClasses.classSelect}[data-id="${
							targetElement.closest(
								selectModule.getSelectClass(
									selectModule.selectClasses.classSelectTag,
								),
							).dataset.selectId
						}"]`,
				  )
			const originalSelect =
				selectModule.getSelectElement(selectItem).originalSelect
			if (targetType === 'click') {
				...
				if (
					...
					targetType === 'focusin'
						? selectItem.classList.add(
								selectModule.selectClasses.classSelectFocus,
						  )
						: selectItem.classList.remove(
								selectModule.selectClasses.classSelectFocus,
						  )
	...
	selectsСlose() {
		const selectActiveItems = document.querySelectorAll(
			`${selectModule.getSelectClass(
				...
				selectModule.selectClasses.classSelectOpen,
			)}`,
		)
	...
	getSelectTitleValue(selectItem, originalSelect) {
		...
		let selectTitleValue = selectModule.getSelectedOptionsData(
			originalSelect,
			2,
		).html
				...
				.elements.map(
					option =>
						`<span role="button" data-select-id="${
							...
							option.value
						}" class="_select-tag">${selectModule.getSelectElementContent(
							option,
						)}</span>`,
				)
				.join('')
		...
		selectModule.getSelectedOptionsData(originalSelect).values.length
			...
			: selectItem.classList.remove(
					selectModule.selectClasses.classSelectActive,
			  )
		...
		if (originalSelect.hasAttribute('data-search')) {
			...
			const customClass =
				...
				selectModule.getSelectedOptionsData(originalSelect).elements[0].dataset
					.class
					? ` ${
							selectModule.getSelectedOptionsData(originalSelect).elements[0]
								.dataset.class
					  }`
					: ''
	...
	getSelectedOptionsData(originalSelect, type) {
		...
		return {
			...
			html: selectedOptions.map(option =>
				selectModule.getSelectElementContent(option),
			),
		}
	},
	...
	getOptions(originalSelect) {
		...
		if (selectOptions.length > 0) {
			...
			selectOptions.forEach(selectOption => {
				...
				selectOptionsHTML += selectModule.getOption(
					selectOption,
					originalSelect,
				)
			})
	...
	selectDisabled(selectItem, originalSelect) {
		if (originalSelect.disabled) {
			...
			selectItem.classList.remove(
				selectModule.selectClasses.classSelectDisabled,
			)
			selectModule.getSelectElement(
				selectItem,
				selectModule.selectClasses.classSelectTitle,
